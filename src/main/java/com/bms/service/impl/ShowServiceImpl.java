...
public class ShowServiceImpl implements ShowService {
	...
	public ShowDto addShow(ShowDto showDto) {
		...
		if (!optionalMovie.isPresent()) {
			throw new BmsException("Movie Not Found with ID: " + showDto.getMovie().getId() + " to add New Show");
		}

		Optional<TheaterEntity> optionalTheater = theaterRepository.findById(showDto.getTheatre().getId());

		if (!optionalTheater.isPresent()) {
			throw new BmsException("Theater Not Found with ID: " + showDto.getMovie().getId() + " to add New Show");
		}
